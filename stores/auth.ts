...
export const useAuth = defineStore('auth', {
  ...
  actions: {
    ...
    async updatePass(payload:any){
      const url="https://storytime-api.strapi.timedoor-js.web.id/api/users/me/reset-password"
      const cookie=useCookie('token')
      const token=cookie.value
    ...
    async uploadProfilePicture(payload:any){
      ...
      const formData=new FormData()
      formData.append('files',payload.imageFiles)
      formData.append('refId',payload.id)
      formData.append('ref',"plugin::users-permissions.user")
      formData.append('field',"profile_picture")
      
      try{
        const uploadUserImage=await axios.post(url,formData,{headers:{
        Authorization: "Bearer " + token,
        'content-type':'multipart/form-data'
      }})
    ...
    async remove_Profile_Picture(picture_id:any){
      const url=`https://storytime-api.strapi.timedoor-js.web.id/api/upload/files/${picture_id}`
      const cookie=useCookie('token')
      const token=cookie.value
