...
export const useStory = defineStore("story", {
  ...
  getters:{
    isBookmarked:(state) =>(id:any)=>{
      return state.bookmark.some(item=>item.id===id)
    },
  },
  ...
  actions: {
    ...
    async fetchStoryList(key: any, author: any, page: any, isEdit: any,isSort:any) {
      ...
      try {
        const story = await axios.get(
          `https://storytime-api.strapi.timedoor-js.web.id/api/stories/?keyword=${key}&author=${author}&page=${page}&sort=${isSort}`
        );
        console.log("nilai sort:",isSort);
    ...
    async bookmarkStory(payload: any) {
      ...
      const index = this.bookmark.findIndex(story => story.id === storydata.id)
      if (index===-1) {
